...
class FramsticksCLI:
    ...
    _next_instance_id = count(
        0
    )  # "static" counter incremented when a new instance is created. Used to ensure unique filenames for each instance.
    ...
    def __runCommand(self, command, genotypes, result_file_name, saveformat) -> List[str]:
        ...
        if saveformat == self.GENO_SAVE_FILE_FORMAT["RAWGENO"]:
            for i in range(len(genotypes)):
                ...
                rel, abs = self.__saveGenotypeToFile(
                    genotypes[i], "genotype" + str(i) + ".gen", "w", self.GENO_SAVE_FILE_FORMAT["RAWGENO"]
                )
                filenames_rel.append(rel)
                filenames_abs.append(abs)
        elif saveformat == self.GENO_SAVE_FILE_FORMAT["NATIVEFRAMS"]:
            self.__saveGenotypeToFile(
                None, self.CLI_INPUT_FILE, "d", None
            )  # 'd'elete: ensure there is nothing left from the last run of the program because we "a"ppend to file in the loop below
            for i in range(len(genotypes)):
                rel, abs = self.__saveGenotypeToFile(
                    genotypes[i], self.CLI_INPUT_FILE, "a", self.GENO_SAVE_FILE_FORMAT["NATIVEFRAMS"]
                )
    ...
    sendDirectCommand_counter = count(
        0
    )  # an internal counter for the sendDirectCommand() method; should be static within that method but python does not allow
    ...
    def getSimplest(self, genetic_format) -> str:
        files = self.__runCommand(
            self.GETSIMPLEST_CMD + " " + genetic_format + " ", [], self.GETSIMPLEST_FILE, self.GENO_SAVE_FILE_FORMAT["RAWGENO"]
        )
    ...
    def evaluate(self, genotype_list: List[str]):
        ...
        if len(data) > 0:
            self.__cleanUpCommandResults(files)
            assert len(genotype_list) == len(data), (
                f"After evaluating {len(genotype_list)} genotype(s) got {len(data)} result(s)."
            )
            return data
        else:
    ...
    def crossOver(self, genotype_parent1: str, genotype_parent2: str) -> str:
        ...
        """
        files = self.__runCommand(
            self.CROSSOVER_CMD, [genotype_parent1, genotype_parent2], self.CROSSOVER_FILE, self.GENO_SAVE_FILE_FORMAT["RAWGENO"]
        )
        with open(files[-1]) as f:
    ...
    def dissimilarity(self, genotype_list: List[str]) -> np.ndarray:
        ...
        assert (square_matrix == square_matrix.T).all(), (
            "Probably not a correct dissimilarity matrix, expecting symmetry, verify this"
        )
    ...
    def isValid(self, genotype_list: List[str]) -> List[bool]:
        ...
        self.__cleanUpCommandResults(files)
        assert len(genotype_list) == len(valid), "Submitted %d genotypes, received %d validity values" % (
            len(genotype_list),
            len(valid),
        )
        return valid
...
def parseArguments():
    ...
    parser.add_argument(
        "-exe",
        required=False,
        help='Executable name. If not given, "frams.exe" or "frams.linux" is assumed depending on the platform.',
    )
    parser.add_argument(
...
if __name__ == "__main__":
    ...
    print(
        'Sending a direct command to Framsticks CLI that calculates "4"+2 yields',
        repr(framsCLI.sendDirectCommand('Simulator.print("4"+2);')),
    )
